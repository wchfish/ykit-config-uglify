...
process.on('message', function(m) {
    ...
    const assetSourceMapName = `${assetName}.map`;
    let response = {};
    ...
    if(/\.js$/.test(assetName) || /\.css$/.test(assetName)) {
        ...
        if(path.extname(assetName) === '.js') {
            ...
            try {
                ...
                // 使用uglifyjs2进行压缩，并生成对应的source map
                result = UglifyJS.minify(content, {
                    fromString: true,
                    mangle: willMangle,
                    outSourceMap: assetSourceMapName,
                    outFileName: `${assetName}`
                });
                minifiedCode = result && result.code;
                minifiedSourceMap = (result && result.map) || null;
            } catch(e) {
        ...
        if(minifiedCode) {
            ...
            minifiedSourceMap && fs.writeFileSync(path.resolve(cwd, assetSourceMapName), minifiedSourceMap, {encoding: 'utf8'});
